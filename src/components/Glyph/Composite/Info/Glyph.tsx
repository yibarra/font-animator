...
const Glyph = ({ rotation, x, y }: { rotation: number, y: number, x: number}) => {
  ...
  const { data: { charIndex, properties: { fontSize }}, path: { bounding } } = UseGlyphContext()
  ...
  return (
    <Group x={bounding.x2 + 40} y={(bounding.y2 - bounding.y1) - 70}>
      <Text {...props} text="Char Index:" y={0} />
      <Text {...props} {...propsBold} text={charIndex.toString()} x={70} y={-2} />
      <Text {...props} text="Rotation:" y={16} />
      <Text {...props} {...propsBold} text={Math.abs(rotation).toFixed(0)} x={70} y={14} />
      <Text {...props} text="deg" x={86} y={16} />
      <Text {...props} text="Position:" y={32} />
      <Text {...props} {...propsBold} text={`${x}:${y}`} x={70} y={30} />
